...
def get_dowker_complex(N,cutoff,filtration,PI,LI):
    ...
    name = all_data[N]
    filename = pre + 'pocket_coordinate/' + name + '_' + Protein_Atom[PI] + '_' + Ligand_Atom[LI] + '_coordinate.csv'
    point_cloud = np.loadtxt(filename,delimiter=',')
    filename = pre + 'pocket_coordinate/' + name + '_' + Protein_Atom[PI] + '_' + Ligand_Atom[LI] + '_protein_ligand_number.csv'
...
def eigenvalue0_of_each_combination_to_file(typ,simplices,name,P,L,cutoff,filtration,grid):
    ...
    pre1 = pre + 'eigenvalue_' + str(cutoff) + '_' + str(filtration) + '_'  + 'zero/'
...
def train_feature_to_file(start,end,cutoff,filtration,grid):
    row = end - start
    N = 11
    ...
    pre1 = pre + 'eigenvalue_' + str(cutoff) + '_' + str(filtration) + '_zero/' 
    add_value = 1
    ...
    filename = pre + 'pocket_feature/dis_train.csv'
...
def test_feature_to_file(start,end,cutoff,filtration,grid):
    row = end - start
    N = 11
    number0 = int ((filtration-2)/0.1 )
    number1 = int ((filtration-2)/0.1)
    ...
    filename = pre + 'pocket_feature/dis_test.csv'
    np.savetxt(filename,feature_matrix,delimiter=',')
...
def get_pearson_correlation(typ,pref):
    feature_matrix_of_train = np.loadtxt( pre + 'pocket_feature/' + 'dis_train.csv',delimiter=',' )
    target_matrix_of_train = np.loadtxt( pre + 'pocket_feature/' + 'target_matrix_of_train.csv',delimiter=',' )
    feature_matrix_of_test = np.loadtxt( pre + 'pocket_feature/' + 'dis_test.csv',delimiter=',' )
...
def run_for_PDBbind_2016():
    ##############################################################
    '''
    by running this function, you can get the results for data2016
    '''
    ##############################################################
    
    
    # extract coordinate
    pocket_coordinate_data_to_file(0,4057) 
    
    # create dowker complex and compute the spectral information
    eigenvalue_to_file(0,4057,10,10,0.1)
    
    
    # feature generation
    train_feature_to_file(0,3772,10,10,0.1)
    test_feature_to_file(0,285,10,10,0.1)
    get_target_matrix_of_train()
    get_target_matrix_of_test()
    
    # machine learning
    get_pearson_correlation('distance','')
    
    
run_for_PDBbind_2016()
