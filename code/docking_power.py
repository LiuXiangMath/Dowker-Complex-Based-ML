...
f1 = open(pre + 'test_data_' + Year + '.txt')
...
def test_coordinate_data_to_file(start,end):
    ...
    for i in range(start,end):
        ...
        # test ligand
        t2 = pre + 'docking_test/' + name + '_decoys.mol2'
        f2 = open(t2,'r')
        contents = f2.readlines()
        f2.close()
        test_index_list = [0]
        for ii in range(len(contents)):
            if contents[ii][0:13]=='@<TRIPOS>BOND':
                test_index_list.append(ii+1)
        for temp_i in range(len(test_index_list)-1):
            top = test_index_list[temp_i]
            bottom = test_index_list[temp_i+1]
...
def train_coordinate_data_to_file(start,end):
    ...
    for i in range(start,end):
        ...
        # train ligand
        t2 = pre + 'docking_train/poses/' + name + '_training_poses.sdf'
        f2 = open(t2,'r')
        contents = f2.readlines()
        f2.close()
        train_index_list = [0]
...
def get_dowker_complex(typ,N,pose_name,cutoff,filtration,PI,LI):
    ####################################################################################
    '''
    this function create the filtered dowker complex of the bipartite graph
    (1) N is the index of the data we deal with
    (2) cutoff determines the binding core region we extract
    (3) filtration is the max_filtration we cover
    (4) PI and LI are the index of specific atom-combination we deal with
    '''
    ####################################################################################
...
def test_eigenvalue_to_file(start,end,cutoff,filtration,grid):
    for i in range(start,end):
        ...
        filename = pre + '/docking_test/' + name + '_rmsd.dat'
        f = open(filename)
        contents = f.readlines()
        f.close()
        for j in range(len(contents)):
            temp = contents[j].split()
            pose_name = temp[0]
...
def before_feature(start,end):
    ...
    create_coordinate_dir(start,end)
    test_coordinate_data_to_file(start,end)
    print('test coordinate ok')
    train_coordinate_data_to_file(start,end)
    print('train coordinate ok')
    
    create_eigenvalue_dir(start,end)
    print('start generate eigenvalue')
    test_eigenvalue_to_file(start,end,10,10,0.1)
    
    print('test eigenvalue ok')
    train_eigenvalue_to_file(start,end,10,10,0.1)
...
def train_feature_to_file(name,cutoff,filtration,grid):
    ...
    filename = pre + 'dis_feature/' + name + '_dis_train_' + str(grid) + '.npy'
...
def test_feature_to_file(name,cutoff,filtration,grid):
    ...
    filename = pre + 'dis_feature/' + name + '_dis_test_' + str(grid) + '.npy'
    np.save(filename,feature_matrix)



def train_label_to_file(name):
    filename = pre + 'docking_train/INDEX/' + name + '.data'
    f = open(filename)
    contents = f.readlines()
    f.close()
    temp = []
    ...
    res = np.array(temp)
    filename = pre + 'dis_feature/' + name + '_train_label.npy'
...
def predict(start,end):
    for i in range(start,end):
        name = test_data[i]
        #print(i,name)
        filename1 = pre + 'dis_feature/' + name + '_dis_train_0.1.npy'
        train_feature = np.load(filename1)
        
        filename2 = pre + 'dis_feature/' + name + '_train_label.npy'
        train_label = np.load(filename2)
        filename3 = pre + 'dis_feature/' + name + '_dis_test_0.1.npy'
        test_feature = np.load(filename3)
        
        #print(train_feature.shape,test_feature.shape,train_label.shape)
        final_res = 0
        for j in range(10):
            ...
            #index = sorted(pre_sort,key=lambda x:(x[0]))
            ...
            #print('index before: ',top1_index,top5_index,top10_index,same_number)
            if same_number==0:
                top1_index = 195
            
            
            filename4 = pre + 'decoys_docking/' + name + '_rmsd.dat'
            f = open(filename4)
            ref = f.readlines()
            f.close()
            ...
            for k in range(top1_index):
                index1 = index[k][1]
                temp2 = ref[index1].split()
                value = float(temp2[2])
                if value<2:
                    final_res = final_res + 1
                    break
            print(j,'top1 index: ',final_res)
            if final_res>5:
                break
        print(i,name,'rate:',final_res)
